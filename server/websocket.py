"""WebSocket 处理：/ws/app（iOS）+ /ws/agent（Win agent）。

- max_msg_size = 16MB
- ping 间隔 20s
- iOS WS 走 user_token 鉴权，agent WS 走 AGENT_TOKEN（hmac.compare_digest）
"""
...
import auth
import broker
from config import WS_MAX_MSG_SIZE, WS_PING_INTERVAL
...
async def ws_app_handler(request: web.Request) -> web.WebSocketResponse:
    ...
    logger.info("iOS WS 连接: user=%s", payload.get("uid"))

    # 连接即推一次状态
    try:
        await ws.send_str(
            json.dumps(
                {"type": "status_update", "data": broker.get_status_payload(), "ts": int(time.time())},
                ensure_ascii=False,
            )
        )
    except Exception:
...
async def _handle_app_text(ws: web.WebSocketResponse, raw: str) -> None:
    """iOS 下行：ping / send_message / browse_files / read_file / ..."""
    try:
        data = json.loads(raw)
    ...
    if msg_type in (
        ...
        try:
            ...
            if not ok:
                ...
                await ws.send_str(
                    json.dumps(
                        {"type": "ack", "data": {"ok": True, "queued": True, "request_id": forward["request_id"]}, "ts": int(time.time())},
                        ensure_ascii=False,
                    )
                )
        except Exception as e:
            await ws.send_str(
                json.dumps({"type": "error", "data": {"msg": str(e)}, "ts": int(time.time())}, ensure_ascii=False)
            )
        return
    ...
    try:
        async for msg in ws:
            if msg.type == WSMsgType.TEXT:
                ...
                logger.debug("Agent binary %d bytes，忽略", len(msg.data))
            elif msg.type == WSMsgType.ERROR:
